...
def test_provisioning_removes_legacy_grants_and_is_repeatable(
    ...
    for database_name in ("space_corp", "space_corp_test"):
        subprocess.run(
            [str(psql), "-X", "-v", "ON_ERROR_STOP=1", "-d", database_name, "-c",
    ...
    for database_name in ("space_corp", "space_corp_test"):
        result = subprocess.run(
            [str(psql), "-X", "-At", "-v", "ON_ERROR_STOP=1", "-d", database_name, "-c",
    ...
    for privilege in ("SELECT", "INSERT"):
        assert sql(
            provisioning_cluster,
            "SELECT has_table_privilege("
            f"'space_corp_app', 'work_orders', '{privilege}')",
        ) == "t"
    ...
    for column in (
        ...
        "target_equipment_unit_id", "reference_code",
    ):
        assert sql(
            provisioning_cluster,
            "SELECT has_column_privilege("
...
def test_operations_restricted_role_isolation_and_fixed_fields(
    ...
    owner_url = URL.create(
        "postgresql+psycopg", username="space_corp", database="space_corp",
        host="localhost", query={"host": environment["PGHOST"], "port": environment["PGPORT"]},
    )
    owner = Database(owner_url.render_as_string(hide_password=False))
    application = Database(owner_url.set(username="space_corp_app").render_as_string(hide_password=False))
    ...
    try:
        with owner.session() as session:
            ...
            session.flush()
            session.add(EquipmentModelRecord(id=model_id, catalog_release_id=release_id, code="ECS", name="ECS"))
            for workspace_id, facility_id in zip(workspace_ids, facility_ids, strict=True):
                session.add(FacilityRecord(
                    id=facility_id, workspace_id=workspace_id, code="FAC-01", name="Facility",
                    facility_type="lunar_installation", location="Moon", operational_status="operational",
                ))
            session.flush()
            for workspace_id, facility_id, unit_id in zip(workspace_ids, facility_ids, unit_ids, strict=True):
                session.add(EquipmentUnitRecord(
                    id=unit_id, workspace_id=workspace_id, facility_id=facility_id,
                    equipment_model_id=model_id, asset_tag="UNIT-01", operational_status="operational",
                ))
        assert provision(provisioning_cluster).returncode == 0
        ...
        for index, workspace_id in enumerate(workspace_ids):
            with application.workspace_session(workspace_id) as session:
                session.add(IncidentRecord(
                    id=incident_ids[index], workspace_id=workspace_id,
                    facility_id=facility_ids[index], equipment_unit_id=unit_ids[index],
                    reference_code="INC-01", severity="high", status="open",
                    occurred_at=datetime(2026, 1, 1, tzinfo=UTC), fault_code="AIRFLOW",
                ))
                session.flush()
                session.add(WorkOrderRecord(
                    id=work_order_ids[index], workspace_id=workspace_id,
                    facility_id=facility_ids[index], originating_incident_id=incident_ids[index],
                    target_equipment_unit_id=unit_ids[index], reference_code="WO-01",
        ...
        with application.workspace_session(workspace_ids[0]) as session:
            ...
            for table, foreign_id in (("incidents", incident_ids[1]), ("work_orders", work_order_ids[1])):
                result = session.execute(text(f"UPDATE {table} SET status = 'open' WHERE id = :id"), {"id": foreign_id})
                assert result.rowcount == 0

            resolved = SqlAlchemyIncidentRepository(session).update_lifecycle(
                workspace_ids[0], incident_ids[0], severity=IncidentSeverity.CRITICAL,
                status=IncidentStatus.RESOLVED, resolved_at=datetime.now(UTC),
            )
            completed = SqlAlchemyWorkOrderRepository(session).update_lifecycle(
                workspace_ids[0], work_order_ids[0], priority=WorkOrderPriority.CRITICAL,
                status=WorkOrderStatus.COMPLETED, due_at=datetime.now(UTC), completed_at=datetime.now(UTC),
            )
            ...
            fixed_columns = {
                "incidents": ("id", "workspace_id", "facility_id", "equipment_unit_id", "reference_code", "occurred_at", "fault_code", "created_at"),
                "work_orders": ("id", "workspace_id", "facility_id", "originating_incident_id", "target_equipment_unit_id", "reference_code", "created_at"),
                "equipment_units": ("id", "workspace_id", "facility_id", "equipment_model_id", "asset_tag", "created_at"),
            }
            for table, columns in fixed_columns.items():
                for column in columns:
                    assert_sqlstate(session, f"UPDATE {table} SET {column} = {column}", "42501")
            for table in ("facilities", "equipment_units", "inventory_items", "incidents", "work_orders"):
            ...
            for template in (incident_insert, work_insert):
                values = dict(id=uuid4(), workspace=workspace_ids[1], facility=facility_ids[1], unit=unit_ids[1], incident=incident_ids[1])
            ...
            for field in ("unit", "incident"):
                values = dict(id=uuid4(), workspace=workspace_ids[0], facility=facility_ids[0], unit=unit_ids[0], incident=incident_ids[0])
        ...
        with owner.session() as session:
            session.execute(text("UPDATE work_orders SET reference_code = 'REPAIRED' WHERE id = :id"), {"id": work_order_ids[0]})
