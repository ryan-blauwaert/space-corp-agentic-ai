...
def test_equipment_units_read_contract(read_api_dataset: ReadApiDataset) -> None:
    ...
    assert [item["id"] for item in items] == [str(row["id"]) for row in sorted(expected, key=lambda row: (row["asset_tag"], row["id"]))]
    ...
    combined = {field: items[0][field] for field in ['facility_id', 'equipment_model_id', 'operational_status'] if items[0][field] is not None}
    ...
    for record_id in (other_item["id"], str(uuid4())):
        response = client.get(f"/equipment-units/{record_id}", headers={"X-Workspace-ID": str(data.other_id)})
    ...
    assert client.get("/equipment-units", params={"workspace_id": str(data.other_id)}).status_code == 422
    assert client.get("/equipment-units", headers={"X-Workspace-ID": str(data.other_id)}).json()["items"] == items[:50]
...
@pytest.mark.parametrize("suffix", ["/not-a-uuid", "?limit=0", "?limit=101", "?offset=-1", "?facility_id=invalid", "?workspace_id=other", '?equipment_model_id=invalid', '?operational_status=unknown'])
