...
def test_incidents_read_contract(read_api_dataset: ReadApiDataset) -> None:
    ...
    assert [item["id"] for item in items] == [str(row["id"]) for row in sorted(expected, key=lambda row: (row["occurred_at"], row["id"]))]
    ...
    combined = {field: items[0][field] for field in ['facility_id', 'equipment_unit_id', 'status', 'fault_code'] if items[0][field] is not None}
    ...
    for record_id in (other_item["id"], str(uuid4())):
        response = client.get(f"/incidents/{record_id}", headers={"X-Workspace-ID": str(data.other_id)})
    ...
    assert client.get("/incidents", headers={"X-Workspace-ID": str(data.other_id)}).json()["items"] == items[:50]
    assert client.post("/incidents", json={}).status_code == 405
    assert client.patch(f"/incidents/{items[0]['id']}", json={}).status_code == 405


@pytest.mark.parametrize("suffix", ["/not-a-uuid", "?limit=0", "?limit=101", "?offset=-1", "?facility_id=invalid", "?workspace_id=other", '?equipment_unit_id=invalid', '?status=unknown', '?fault_code=', '?occurred_from=2026-01-01T00:00:00', '?occurred_from=2026-02-01T00:00:00Z&occurred_before=2026-01-01T00:00:00Z'])
...
def test_incident_time_window_and_foreign_unit(read_api_dataset: ReadApiDataset) -> None:
    ...
    response = data.client.get("/incidents", params={
        "occurred_from": start.isoformat(), "occurred_before": end.isoformat(), "limit": 100,
    })
    assert response.status_code == 200
    ...
    for field, boundary, predicate in (
        ...
        result = data.client.get("/incidents", params={field: boundary.isoformat(), "limit": 100}).json()
        assert {item["id"] for item in result["items"]} == {str(row["id"]) for row in rows if predicate(row["occurred_at"])}
    other_unit = data.other_client.get("/equipment-units").json()["items"][0]["id"]
    assert data.client.get("/incidents", params={"equipment_unit_id": other_unit}).json()["pagination"]["total"] == 0
    fault = next(row["fault_code"] for row in rows if row["fault_code"])
    assert data.client.get("/incidents", params={"fault_code": f" {fault.lower()} "}).json() == data.client.get("/incidents", params={"fault_code": fault}).json()
