...
def test_work_orders_read_contract(read_api_dataset: ReadApiDataset) -> None:
    ...
    assert [item["id"] for item in items] == [
        str(row["id"])
        for row in sorted(expected, key=lambda row: (row["reference_code"], row["id"]))
    ]
    ...
    for record_id in (other_item["id"], str(uuid4())):
        response = client.get(
            f"/work-orders/{record_id}", headers={"X-Workspace-ID": str(data.other_id)}
        )
    ...
    assert (
        client.get("/work-orders", params={"workspace_id": str(data.other_id)}).status_code == 422
    )
    assert (
        client.get("/work-orders", headers={"X-Workspace-ID": str(data.other_id)}).json()["items"]
        == items[:50]
    )
