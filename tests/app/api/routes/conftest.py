...
from app.equipment.models import CatalogReleaseRecord, ComponentRecord, EquipmentModelRecord, equipment_model_components
...
@pytest.fixture
def read_api_dataset(migrated_database: str, integration_application_database_url: str) -> Iterator[ReadApiDataset]:
    ...
    def client(workspace: UUID) -> TestClient:
        return TestClient(create_app(Settings(
            _env_file=None, database_url=integration_application_database_url,
    ...
    try:
        ...
        with client(first) as primary, client(other) as secondary, client(empty) as vacant:
            yield ReadApiDataset(
                primary, secondary, vacant, first, other, manifest,
                {record.__tablename__: rows for record, rows in baseline_rows(manifest, first)},
            )
    finally:
        with owner.session() as session:
            for record in reversed(OPERATIONAL):
                session.execute(delete(record).where(record.workspace_id.in_([first, other, empty])))
            session.execute(delete(WorkspaceRecord).where(WorkspaceRecord.id.in_([first, other, empty])))
            session.execute(delete(BaselineRecord).where(BaselineRecord.catalog_release_id == release))
            session.execute(delete(equipment_model_components).where(equipment_model_components.c.catalog_release_id == release))
            session.execute(delete(EquipmentModelRecord).where(EquipmentModelRecord.catalog_release_id == release))
            session.execute(delete(ComponentRecord).where(ComponentRecord.catalog_release_id == release))
