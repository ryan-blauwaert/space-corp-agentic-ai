...
def make_work_order(
    workspace_id: UUID, facility_id: UUID, **overrides: object
) -> WorkOrderRecord:
    values = {
        "workspace_id": workspace_id, "facility_id": facility_id,
        "originating_incident_id": None, "target_equipment_unit_id": None,
        "reference_code": "WO-LUN-001", "priority": "high", "status": "open",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"resolved_at": datetime(2026, 1, 2, tzinfo=UTC)}, "ck_incidents_resolved_at_matches_status"),
        ({"status": "resolved", "resolved_at": datetime(2025, 12, 31, tzinfo=UTC)}, "ck_incidents_resolved_at_after_occurred_at"),
    ],
)
def test_incident_rejects_invalid_lifecycle_and_fault_code(
    ...
    integration_session.add(
        make_incident(
            workspace.id, facility.id, None, occurred_at=datetime(2026, 1, 1, tzinfo=UTC), **overrides
        )
    )
...
def test_work_order_relationships_preserve_origin_and_target_meanings() -> None:
    ...
    assert IncidentRecord.__mapper__.relationships["originating_work_orders"].mapper.class_ is WorkOrderRecord
    assert EquipmentUnitRecord.__mapper__.relationships["targeted_work_orders"].mapper.class_ is WorkOrderRecord
    assert WorkOrderRecord.__mapper__.relationships["originating_incident"].mapper.class_ is IncidentRecord
    assert WorkOrderRecord.__mapper__.relationships["target_equipment_unit"].mapper.class_ is EquipmentUnitRecord
...
def test_work_order_rejects_reference_from_a_different_facility(
    ...
    integration_session.flush()
    values = {"originating_incident_id": incident.id} if reference_kind == "incident" else {"target_equipment_unit_id": unit.id}
...
def test_work_order_accepts_independent_reference_combinations(
    ...
    records = [
        ...
        make_work_order(workspace.id, facility.id, reference_code="WO-002", target_equipment_unit_id=unit.id),
        make_work_order(workspace.id, facility.id, reference_code="WO-003", originating_incident_id=incident.id),
        make_work_order(workspace.id, facility.id, reference_code="WO-004", originating_incident_id=incident.id, target_equipment_unit_id=unit.id),
        make_work_order(workspace.id, facility.id, reference_code="WO-005", originating_incident_id=incident.id, target_equipment_unit_id=target.id),
    ]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"completed_at": datetime(2026, 1, 2, tzinfo=UTC)}, "ck_work_orders_completed_at_matches_status"),
        ({"status": "cancelled", "completed_at": datetime(2026, 1, 2, tzinfo=UTC)}, "ck_work_orders_completed_at_matches_status"),
        ({"status": "completed", "completed_at": datetime(2025, 12, 31, tzinfo=UTC)}, "ck_work_orders_completed_at_after_created_at"),
    ],
)
...
def test_work_order_completion_boundary_and_scoped_uniqueness(
    ...
    for facility in facilities:
        integration_session.add(make_work_order(
            facility.workspace_id, facility.id, status="completed",
            created_at=instant, completed_at=instant,
        ))
    integration_session.flush()
