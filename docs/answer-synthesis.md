...
- Fault codes retain the original quoting and escaping for control characters,
  ...
  escaped evidence into active markup. Version-2 display labels use JSON-style quoted
  plain text with escaped control characters (see below), not HTML entities. The
  renderer only uses stored names; it does not invent human-readable identities.
...
The current renderer version is `2`; frozen version-1 protocols and reports are
preserved as historical evidence, not relabeled as validation of this change.
